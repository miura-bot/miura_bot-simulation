...
# start_orientation_1 = p.getQuaternionFromEuler([np.pi, 0., 0])
...
while True:
    ...
    if not vector_done and not np.isclose(np.add(pos0, target_pos), pos1, atol=0.1).all():
        ...
        # s1 = (vector_to_follow[1] + (np.sqrt(3)/3.) * vector_to_follow[0]) * -10.0 * 2.0
        # s2 = 2.0 * np.sqrt(3)/3. * vector_to_follow[0] * -10.0
        
        # s1 = (magnitude[0] * 5.0) / abs(magnitude[0] + magnitude[1])
        # s2 = (magnitude[1] * 5.0) / abs(magnitude[0] + magnitude[1])
